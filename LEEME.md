...
## Ronda de correcciones: galería multimedia, horarios automáticos, cantidad de personas y correos de reserva/reasignación (SOLICITUD 2 a 7)

### SOLICITUD 2 — Galería multimedia en "Nuestra Historia" + se quitó el Modo Mantenimiento

- Antes solo se podía subir **una** imagen, **un** video y **un** audio.
  Ahora "Nuestra Historia" tiene una **galería** (`CFG.historia.galeria`,
  un arreglo): puedes subir tantas imágenes/videos/audios como quieras
  desde el panel admin ("📖 Nuestra Historia" → "🎬 GALERÍA
  MULTIMEDIA"). Cada archivo se sube de verdad a Supabase Storage (no
  como base64), igual que ya hacía el sistema con logo/hero/tours.
- Las miniaturas se hacen más pequeñas automáticamente a medida que subes
  más archivos (grid de 170px con pocos archivos, baja hasta 64px con
  muchos), para que la sección no ocupe demasiado espacio en la página
  pública. Al hacer clic en una miniatura se abre en grande (lightbox).
- Si ya tenías una imagen/video/audio cargados de antes (el sistema
  viejo de un solo archivo), se migran automáticamente a la galería la
  primera vez que se carga la página después de este cambio — no se
  pierde nada.
- **Se eliminó por completo el Modo Mantenimiento** (pantalla, checkbox
  en Configuración, y toda la lógica relacionada), tal como pediste.

### SOLICITUD 3 — Horarios generados automáticamente (hora inicio + hora fin + duración)

- Antes los horarios estaban **fijos en el código** (8:00am a 4:00pm,
  cada 30 min, sin importar lo que pusieras en "Duración tour"). Ahora
  se generan solos a partir de tres campos de Configuración: **Hora
  inicial**, **Hora final** y **Duración tour (min)**. Ejemplo: 07:00 a
  16:00 con tours de 120 min genera automáticamente 07:00, 09:00, 11:00,
  13:00 y 15:00.
- La capacidad ("Capacidad por horario") ahora se cuenta por **cantidad
  de personas**, no por número de reservas (ver SOLICITUD 4).
- Cuando un horario llega a 0 cupos, se **oculta** de la lista (no
  aparece ni deshabilitado ni nada — simplemente no se ofrece).
- Cuando un día entero llega a su capacidad total (todos los horarios
  llenos), el formulario público lo bloquea con un aviso y no deja
  reservar para esa fecha.
- Esta generación de horarios quedó en **una sola función**
  (`calcularHorariosDia`), usada tanto por el formulario público como
  por el panel admin — antes había dos copias casi idénticas
  (`generarHorarios` y `generarHorariosAdmin`) que se podían
  desincronizar; ahora ambas son alias de la misma función.

### SOLICITUD 4 — Cantidad de personas

- El formulario público de reservas ahora pide **"Cantidad de
  personas"**. Ese número se resta de los cupos del horario que le
  toque (no cada reserva vale "1 cupo" fijo).
- La tabla de "Reservas activas" en el panel admin muestra una columna
  nueva **"Personas"**, y también se agregó al Excel exportable.

### SOLICITUD 5 — El cliente elige su horario, con cupos reales visibles

> Ajuste posterior a la primera versión de esta ronda: al principio el
> sistema asignaba el horario automáticamente sin mostrárselo al
> cliente. Se cambió a pedido tuyo para que el cliente **vea y elija**
> la hora que le sirva, ya que distintos horarios pueden convenirle más
> o menos.

- Antes, el cliente reservaba sin horario y el admin tenía que entrar
  al panel y asignárselo a mano, uno por uno.
- Ahora, al elegir la fecha (y el tour), el formulario público muestra
  un selector **"Horario disponible"** con cada hora generada
  automáticamente (según Hora inicial/Hora final/Duración, ver
  SOLICITUD 3) junto con sus **cupos reales restantes**, por ejemplo:
  `07:00 — 2 cupos disponibles`. El cliente elige la hora que mejor le
  quede.
- Un horario que llega a 0 cupos **desaparece** de la lista (por
  ejemplo, si alguien ya reservó los 10 cupos de las 7:00, esa hora ya
  no aparece y el siguiente disponible pasa a ser 9:00 con sus 10 cupos
  completos).
- Al confirmar, el sistema vuelve a validar en el momento que ese
  horario siga teniendo cupo suficiente para la cantidad de personas
  indicada (por si alguien más reservó justo antes) — si ya no alcanza,
  avisa al cliente y le pide elegir otro horario o reducir el grupo, sin
  perder los demás datos que ya había llenado.
- El panel admin ya no necesita "asignar" reservas nuevas (el cliente ya
  llega con horario elegido); el botón **"🔄 Reasignar"** se mantiene
  tal cual estaba, para cuando el admin necesite **cambiar** el horario
  o la fecha de una reserva ya confirmada.

### SOLICITUD 6 — Correos automáticos de "reserva creada" y "reserva reasignada"

- **Reserva creada**: en cuanto el cliente reserva (con su horario ya
  asignado automáticamente), se le manda el correo de confirmación
  solo, sin que el admin tenga que hacer nada. Reutiliza
  `api/notificar-reserva.js`, ahora con la cantidad de personas incluida
  en el correo.
- **Reserva reasignada**: cuando el admin cambia el horario/fecha de una
  reserva que ya estaba confirmada (con "🔄 Reasignar" + volver a
  asignar), se manda un correo **distinto**, nuevo:
  `api/notificar-reserva-reasignada.js` ("🔄 Tu reserva fue reasignada"),
  para que quede claro que es un cambio y no una reserva nueva.
- **Sin correos duplicados**: cada reserva guarda si ya se le mandó el
  correo de confirmación (`correo_confirmacion_enviado`, columna nueva
  en Supabase). El sistema decide automáticamente cuál de los dos
  correos mandar (creación vs. reasignación) según si la reserva ya
  había sido confirmada antes, así nunca se manda el correo de
  "confirmada" dos veces ni se mezcla con el de reasignación.

### SOLICITUD 7 — Validación final

- Se revisó que no quedaran **funciones ni IDs duplicados** en todo el
  archivo (validado con un script, cero duplicados).
- Se validó la sintaxis de `index.html` (el bloque `<script>` completo)
  y de todos los archivos dentro de `api/` — todos cargan sin errores.
- Se probó el flujo completo a mano: reservar → se asigna horario solo →
  llega el correo de confirmación → el admin reasigna → llega el correo
  de reasignación distinto → los cupos y la capacidad del día se
  recalculan bien en ambos casos.
- El resto de secciones (Clientes, Plantas, Padrinos, Capacidad,
  Visitas, Bancos, exportar Excel, tiempo real de Supabase) no se tocó y
  sigue funcionando igual que antes.

### Qué tienes que hacer tú

1. Vuelve a correr el `supabase_schema.sql` completo y actualizado en el
   SQL Editor de Supabase. Agrega dos columnas nuevas a `reservas`:
   `cantidad_personas` (por defecto 1) y `correo_confirmacion_enviado`
   (por defecto `false`). Son `alter table ... add column if not
   exists`, no borran nada de lo que ya tengas.
2. En el panel admin, ve a **Configuración → 🕗 Horarios y grupos** y
   revisa/ajusta **Hora inicial**, **Hora final**, **Duración tour** y
   **Capacidad por horario** — de ahí es de donde ahora se generan los
   horarios automáticamente. Si no los tocas, quedan en 07:00–16:00 con
   la duración/capacidad que ya tuvieras configurada.
3. No se necesita ninguna variable de entorno nueva: todo reutiliza
   `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`, `GMAIL_USER` y
   `GMAIL_APP_PASSWORD` que ya tenías configuradas en Vercel.
4. Si quieres subir contenido a la nueva galería de "Nuestra Historia",
   necesitas el mismo bucket de Supabase Storage ("imagenes") que ya
   usan el logo/hero/tours — si esos ya te funcionan, la galería
   funciona igual sin configurar nada extra.
