...
create table if not exists public.reservas (
  ...
  correo_confirmacion_enviado boolean not null default false
);

-- Por si la tabla ya existía de antes de agregar el recordatorio automático
-- (correr este archivo completo es seguro las veces que quieras).
alter table public.reservas add column if not exists recordatorio_enviado boolean not null default false;

-- SOLICITUD 4/5/6: cantidad de personas por reserva (usada para calcular
-- cupos por horario) y bandera de si ya se envió el correo de "reserva
-- creada" (para no duplicarlo si se reasigna después).
alter table public.reservas add column if not exists cantidad_personas integer not null default 1;
alter table public.reservas add column if not exists correo_confirmacion_enviado boolean not null default false;
...
-- ══════════════════════════════════════════════════════════
-- Tiempo real: permite que los cambios en configuracion_sitio
-- (Nuestra Historia, imágenes, cfg_general, etc.) se reflejen
-- al instante en todos los navegadores conectados, sin recargar.
-- Seguro de correr varias veces (si ya estaba agregada, ignora el error).
-- ══════════════════════════════════════════════════════════
do $$
begin
