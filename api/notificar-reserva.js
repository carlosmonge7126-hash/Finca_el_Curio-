...
// Si la imagen del hero (base64) es demasiado pesada, no la incrustamos en
// el correo: infla el tamaño del mensaje, lo hace más lento de enviar/abrir
// y muchos filtros de spam penalizan correos con imágenes gigantes inline.
const LIMITE_HERO_BASE64 = 700000; // ~500KB de imagen real

function plantillaCorreo({ nombre, fecha, horario, tour, lang, telefonoFinca, heroImg }) {
  ...
  // Fondo del encabezado: si hay imagen del hero (y no es demasiado pesada),
  // se usa como fondo con una capa verde translúcida encima (igual estilo
  // que el hero del sitio) para que el texto siga siendo legible. Si no hay
  // imagen o es muy pesada, cae al degradado verde de siempre.
  const usarHero = heroImg && typeof heroImg === 'string' && heroImg.length < LIMITE_HERO_BASE64;
  const fondoHeader = usarHero
    ? `background-image:linear-gradient(135deg,rgba(13,59,30,.82),rgba(31,91,50,.72)),url('${heroImg}');background-size:cover;background-position:center;`
    : `background:linear-gradient(135deg,#0d3b1e,#1f5b32);`;
  ...
  <div style="font-family:Georgia,'Times New Roman',serif;max-width:520px;margin:0 auto;background:#fdfbf6;border-radius:16px;overflow:hidden;border:1px solid #e4e0d4;">
    <div style="${fondoHeader}padding:28px 24px;text-align:center;">
      <h1 style="color:#fff;margin:0;font-size:22px;letter-spacing:.5px;text-shadow:0 2px 10px rgba(0,0,0,.45);">🌿 Finca El Curio</h1>
      <p style="color:#c9dd9c;margin:6px 0 0;font-size:14px;text-shadow:0 2px 8px rgba(0,0,0,.45);">${titulo}</p>
    </div>
...
export default async function handler(req, res) {
  ...
  const { email, nombre, fecha, horario, tour, lang, telefonoFinca, heroImg } = req.body || {};
  ...
  try {
    await transporter.sendMail({
      ...
      to: email,
      subject: asunto,
      html: plantillaCorreo({ nombre, fecha, horario, tour, lang, telefonoFinca, heroImg })
    });
