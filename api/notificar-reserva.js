...
function formatearFecha(fechaISO, lang) {
  try {
    ...
    return d.toLocaleDateString(lang === 'en' ? 'en-US' : 'es-CR', {
...
function plantillaCorreo({ nombre, fecha, horario, tour, lang }) {
  ...
  const despedida = es
    ? 'Si tenés alguna pregunta, escribinos por WhatsApp o responde este correo.'
    : "If you have any questions, message us on WhatsApp or reply to this email.";
    ...
    <div style="padding:28px 26px;color:#2b2b2b;">
      ...
      <table style="width:100%;border-collapse:collapse;margin-bottom:22px;">
        <tr>
          ...
          <td style="padding:12px 14px;background:#f1f0e6;border-radius:10px 10px 0 0;font-size:15px;font-weight:bold;color:#0d3b1e;text-align:right;text-transform:capitalize;">${fechaBonita}</td>
        </tr>
        <tr>
...
export default async function handler(req, res) {
  ...
  const { email, nombre, fecha, horario, tour, lang } = req.body || {};
  ...
  try {
    await transporter.sendMail({
      ...
      to: email,
      subject: asunto,
      html: plantillaCorreo({ nombre, fecha, horario, tour, lang })
    });
